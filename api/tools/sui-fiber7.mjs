...
const r = await send("Runtime.evaluate", {
  expression: `(() => {
    ...
    for (let d = 0; f && d < 32; d++, f = f.return) {
      const type = f.type;
      const nm = typeof type === "function" ? (type.displayName || type.name || "fn?") : "";
      const p = f.memoizedProps || {};
      if (nm === "R" || nm === "G" || nm === "so" || nm === "ChainMultiSelect" || nm === "ChainMultiSelectCore") {
        const mini = {};
        for (const k of Object.keys(p)) {
          const v = p[k];
          if (typeof v === "string") mini[k] = v.length > 50 ? v.slice(0, 50) + "..." : v;
          else if (typeof v === "number" || typeof v === "boolean") mini[k] = v;
          else if (Array.isArray(v)) {
            try {
              mini[k] = "ARR[n=" + v.length + "] " + JSON.stringify(v).slice(0, 260);
            } catch { mini[k] = "ARR[n=" + v.length + "] [circ]"; }
          }
        }
        out[nm] = mini;
      }
...
console.log(JSON.stringify(v, null, 1).slice(0, 4000));
ws.close();
process.exit(0);