...
const loc = await send("Runtime.evaluate", {
  expression: `(() => { const a=[...document.querySelectorAll("a[href]")].find(a=>{const h=a.getAttribute("href")||"";return h.includes("/trend?chain=sol")&&a.getBoundingClientRect().width>0&&a.innerText.trim()==="Trending"}); if(!a)return null; a.scrollIntoView({block:"center"}); const r=a.getBoundingClientRect(); return {x:r.x+r.width/2,y:r.y+r.height/2}; })()`,
  returnByValue: true
});
...
console.log("after real click:", after?.result?.value);
