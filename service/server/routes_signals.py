...
def register_signal_routes(app: FastAPI, ctx: RouteContext) -> None:
    ...
    async def push_realtime_signal(data: RealtimeSignalRequest, authorization: str = Header(None)):
        ...
        if data.leverage is not None:
            if market != 'crypto':
                raise HTTPException(status_code=400, detail='Leverage is only supported for crypto (perpetual-futures paper trading).')
            try:
        ...
        if data.stop_loss_pct is not None or data.take_profit_pct is not None:
            if market == 'polymarket':
                raise HTTPException(
                    status_code=400,
                    detail='stop_loss_pct/take_profit_pct are not supported for polymarket positions.',
                )
            if action_lower not in ('buy', 'short'):
                raise HTTPException(
                    status_code=400,
                    detail='stop_loss_pct/take_profit_pct can only be set when opening (buy/short) a position.',
                )
            for name, value in (('stop_loss_pct', data.stop_loss_pct), ('take_profit_pct', data.take_profit_pct)):
            ...
            if action_lower == 'buy':
                if data.stop_loss_pct is not None:
                    stop_loss = price * (1 - data.stop_loss_pct / 100)
                if data.take_profit_pct is not None:
                    take_profit = price * (1 + data.take_profit_pct / 100)
            else:  # short: stop above entry, take below entry
                if data.stop_loss_pct is not None:
                    stop_loss = price * (1 + data.stop_loss_pct / 100)
                if data.take_profit_pct is not None:
                    take_profit = price * (1 - data.take_profit_pct / 100)
        ...
        try:
            ...
            if action_lower in ['buy', 'short']:
                total_deduction = trade_value / leverage + fee
                ...
                if current_cash < total_deduction:
                    raise HTTPException(
                        ...
                        detail=(
                            ...
                            f'(margin: ${trade_value / leverage:.2f} + fee: ${fee:.2f}), Available: ${current_cash:.2f}'
                        ),
                    )
            ...
            if action_lower in ['buy', 'short']:
                # Leveraged crypto (perps): only the margin is deducted from cash.
                margin_deduction = trade_value / leverage + fee
                cursor.execute('UPDATE agents SET cash = cash - ? WHERE id = ?', (margin_deduction, agent_id))
