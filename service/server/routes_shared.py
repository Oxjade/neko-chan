...
# Historical fills: how far back executed_at may go, and how far ahead it may point.
# Prevents "time-travel" trades that backdate fills to past lows with today's knowledge.
MAX_HISTORICAL_FILL_AGE_HOURS = float(os.getenv('MAX_HISTORICAL_FILL_AGE_HOURS', '72'))
MAX_FUTURE_FILL_LEAD_MINUTES = float(os.getenv('MAX_FUTURE_FILL_LEAD_MINUTES', '5'))
...
SUPPORTED_MARKETS = {'us-stock', 'crypto', 'polymarket', 'forex'}
...
def should_fetch_server_trade_price(market: str) -> bool:
    ...
    if normalized_market in {'crypto', 'polymarket', 'us-stock', 'forex'}:
        return True
...
def is_forex_market_open() -> bool:
    """Forex trades ~24/5: closed from Fri 17:00 ET to Sun 17:00 ET."""
    et_tz = ZoneInfo('America/New_York')
    now_et = datetime.now(et_tz)
    day = now_et.weekday()
...
def validate_executed_at(executed_at: str, market: str) -> tuple[bool, str]:
    try:
        ...
        if dt_utc > now_utc + timedelta(minutes=MAX_FUTURE_FILL_LEAD_MINUTES):
            return (
                False,
                f'executed_at is in the future: {executed_at}. '
                f'Fills may not be more than {MAX_FUTURE_FILL_LEAD_MINUTES:.0f} minute(s) ahead of now.',
            )
        if dt_utc < now_utc - timedelta(hours=MAX_HISTORICAL_FILL_AGE_HOURS):
            return (
                False,
                f'executed_at is too far in the past: {executed_at}. '
                f'Historical fills are limited to {MAX_HISTORICAL_FILL_AGE_HOURS:.0f} hour(s) before now '
                'to prevent hindsight/backdated trades.',
            )

        if normalized_market == 'forex':
            minutes_et = dt_et.hour * 60 + dt_et.minute
            if day == 4 and minutes_et >= 17 * 60:
                return False, 'Forex market is closed on Friday after 17:00 ET (closes for the weekend).'
            if day == 5:
                return False, 'Forex market is closed on Saturdays.'
            if day == 6 and minutes_et < 17 * 60:
                return False, 'Forex market reopens Sunday 17:00 ET.'
