...
def get_price_from_market(
    ...
    try:
        ...
        if market == "crypto":
            ...
            price = _get_yfinance_forex_price(symbol, executed_at)
        else:
...
def _normalize_forex_symbol(symbol: str) -> Optional[str]:
    """Normalize a forex pair to yfinance format (EURUSD -> EURUSD=X)."""
    normalized = (symbol or "").strip().upper().replace("/", "").replace("-", "").replace("_", "")
    normalized = normalized.removesuffix("=X").removesuffix("=F")
    if len(normalized) != 6 or not normalized.isalpha():
        return None
    majors = {
        "EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD",
        "NZDUSD", "EURGBP", "EURJPY", "GBPJPY", "EURCHF", "GBPCHF",
        "AUDJPY", "EURAUD", "EURNZD", "USDSEK", "USDNOK", "USDSGD",
        "EURCAD", "CADJPY", "CHFJPY", "AUDCAD", "AUDCHF", "NZDJPY",
        "EURSGD", "GBPAUD", "GBPCAD", "GBPNZD", "NZDCHF", "NZDCAD",
    }
    if normalized not in majors:
        return None
    return f"{normalized}=X"


def _get_yfinance_forex_price(symbol: str, executed_at: str) -> Optional[float]:
    """Best-effort Yahoo Finance price for forex pairs (e.g. EURUSD=X)."""
    yf_symbol = _normalize_forex_symbol(symbol)
    if not yf_symbol:
        _price_log(f"[Price API] Unsupported forex pair: {symbol}")
        return None
    ...
    try:
        ...
        price = _extract_yfinance_close_price(frame, yf_symbol, target_utc)
        if price is not None:
            _price_log(f"[Price API] yfinance forex daily fallback fetched {yf_symbol}: ${price}")
        return price
