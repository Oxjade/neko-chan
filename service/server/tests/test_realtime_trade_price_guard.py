...
class RealtimeTradePriceGuardTests(unittest.TestCase):
    ...
    def test_historical_fill_rejected_when_too_far_in_the_past(self) -> None:
        old = (datetime.now(timezone.utc) - timedelta(days=30)).strftime('%Y-%m-%dT14:00:00Z')
        payload = self._trade_payload()
        payload["executed_at"] = old
        with patch("routes_signals.is_market_open", return_value=True), \
             patch("price_fetcher.get_price_from_market", return_value=403.08):
            response = self.client.post(
                "/api/signals/realtime",
        ...
        self.assertEqual(response.status_code, 400, response.text)
        self.assertIn("too far in the past", response.json()["detail"])
        self.assertEqual(self._counts_and_cash(), (0, 0, 100000.0))

    def test_future_executed_at_rejected(self) -> None:
        future = (datetime.now(timezone.utc) + timedelta(hours=1)).strftime('%Y-%m-%dT%H:%M:%SZ')
        payload = self._trade_payload()
        payload["executed_at"] = future
        with patch("routes_signals.is_market_open", return_value=True), \
             patch("price_fetcher.get_price_from_market", return_value=403.08):
            response = self.client.post(
                "/api/signals/realtime",
        ...
        self.assertEqual(response.status_code, 400, response.text)
        self.assertIn("in the future", response.json()["detail"])
        self.assertEqual(self._counts_and_cash(), (0, 0, 100000.0))

    def test_position_gets_immediate_mark_at_fill(self) -> None:
        with patch("routes_signals.is_market_open", return_value=True), \
             patch("price_fetcher.get_price_from_market", return_value=403.08):
            response = self.client.post(
                "/api/signals/realtime",
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT current_price FROM positions WHERE agent_id = ? AND symbol = 'TSLA'",
            (self.agent_id,),
        )
        row = cursor.fetchone()
        conn.close()
        self.assertIsNotNone(row)
        self.assertEqual(float(row["current_price"]), 403.08)
