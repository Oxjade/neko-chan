...
class ChallengeTests(unittest.TestCase):
    ...
    def test_challenge_apply_fees_matches_live_platform_fee(self):
        challenge = self._create_active_challenge(
            challenge_key="fee-parity",
            rules_json={"apply_fees": True},
        )
        join_challenge(challenge["challenge_key"], self.agent_2)
        ...
        portfolio = get_agent_challenge_portfolio(challenge["challenge_key"], self.agent_2)
        fee = 2.0 * 100.0 * 0.001
        self.assertAlmostEqual(portfolio["portfolio"]["cash"], 1000.0 - 200.0 - fee)

        with patch("price_fetcher.get_price_from_market", return_value=150.0):
            create_challenge_trade(
                challenge["challenge_key"],
                agent_id=self.agent_2,
                data={
