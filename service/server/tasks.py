...
async def update_position_prices():
    ...
    refresh_priced_markets = _env_csv_set("POSITION_PRICE_REFRESH_PRICED_MARKETS", "crypto,us-stock,forex")
                        ...
                        COALESCE(
                            SUM(
                                CASE
                                    ...
                                    WHEN p.side = 'long' THEN ABS(p.quantity) * (p.entry_price / COALESCE(p.leverage, 1) + p.current_price - p.entry_price)
                                    ELSE ABS(p.quantity) * (p.entry_price / COALESCE(p.leverage, 1) + p.entry_price - p.current_price)
                                END
                            ),
                            0
                        ) AS position_value
...
def _execute_risk_close(
    ...
    side = str(position.get("side") or ("long" if qty > 0 else "short"))
    ...
    try:
        ...
        cursor.execute(
            ...
            """,
            (signal_id, agent_id, market, symbol, token_id, outcome, action, entry, abs(qty),
             f"[auto-{reason}] closed at {exit_price:.6f}", timestamp, now_iso, now_iso),
        )
        ...
        conn.commit()
        return {"closed": True, "agent_id": agent_id, "symbol": symbol, "reason": reason, "price": exit_price}
    except Exception as exc:
        conn.rollback()
        print(f"[Risk Close Error] position={position_id} {exc}")
        return {"closed": False, "agent_id": agent_id, "symbol": symbol, "reason": f"error:{exc}"}
    finally:
        conn.close()


def _liquidation_price(position: dict) -> float | None:
    """Simplified perp liquidation price (maintenance margin 0.5% of notional)."""
    try:
    ...
    if lev <= 1:
        return None
    entry = float(position["entry_price"])
    side = str(position.get("side") or ("long" if float(position.get("quantity") or 0) > 0 else "short"))
    if side == "long":
        return entry * (1 - 1 / lev + 0.005)
    return entry * (1 + 1 / lev - 0.005)


def _risk_exit_for(position: dict, price: float):
    """Pure trigger logic: returns (exit_price, reason) or (None, None)."""
...
async def position_risk_management_loop():
    """Auto-close positions when stop-loss / take-profit levels are hit.

    Uses stored positions.current_price (refreshed by the prices task) to avoid
    hammering price providers; falls back to a live quote when the mark is stale.
    """
    from database import get_db_connection
    ...
    while True:
        interval_s = _env_int("POSITION_RISK_INTERVAL", 60, minimum=10)
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(
                ...
                SELECT id, agent_id, symbol, market, token_id, outcome, side, quantity,
                       entry_price, current_price, stop_loss, take_profit, leverage
                FROM positions
                WHERE stop_loss IS NOT NULL OR take_profit IS NOT NULL OR leverage > 1
                """,
            )
            rows = cursor.fetchall()
            conn.close()
            ...
            for row in rows:
                ...
                if price is None:
                    try:
                        price = get_price_from_market(pos["symbol"], now_iso, pos["market"],
                                                      token_id=pos.get("token_id"), outcome=pos.get("outcome"))
                    except Exception:
                        price = None
                if not price or float(price) <= 0:
                    continue

                exit_price, reason = _risk_exit_for(pos, price)
                if exit_price is None:
                    liq_price = _liquidation_price(pos)
                    if liq_price is not None:
                        if str(pos.get("side") or ("long" if float(pos.get("quantity") or 0) > 0 else "short")) == "long":
                            if float(price) <= liq_price:
                ...
                if exit_price is not None:
                    result = await asyncio.to_thread(_execute_risk_close, pos, exit_price, reason, now_iso)
                    if result["closed"]:
                        closed += 1
                        print(f"[Risk Close] agent={result['agent_id']} {result['symbol']} "
                              f"reason={reason} price={exit_price:.4f}")
            if closed:
                print(f"[Risk Close] closed={closed} total_scanned={len(rows)}")
        except Exception as exc:
            print(f"[Risk Management Error] {exc}")

        await asyncio.sleep(interval_s)


async def perp_funding_loop():
    """Charge perp funding on leveraged crypto positions (default: 0.01% per 8h).

    Mirrors real perpetual futures: longs pay when funding is positive, shorts
    receive. Keeps paper perp PnL comparable to real perp markets.
    """
    from database import get_db_connection
    from services import _reserve_signal_id
    from routes_shared import utc_now_iso_z
    ...
    while True:
        interval_s = _env_int("PERP_FUNDING_INTERVAL_SECONDS", 8 * 3600, minimum=300)
        rate = _env_float("PERP_FUNDING_RATE", 0.0001)
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(
                """
                SELECT id, agent_id, symbol, side, quantity, entry_price, leverage
                FROM positions
                WHERE market = 'crypto' AND leverage IS NOT NULL AND leverage > 1
                """,
            )
            rows = cursor.fetchall()
            charged = 0
            for row in rows:
                ...
                notional = abs(qty) * float(pos["entry_price"])
                funding = notional * rate * (1.0 if side == "long" else -1.0)
                cursor.execute("UPDATE agents SET cash = cash - ? WHERE id = ?", (funding, pos["agent_id"]))
                cursor.execute(
                    """
                    INSERT INTO signals
                    ...
                    VALUES (?, ?, 'operation', 'crypto', 'realtime', ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        _reserve_signal_id(cursor),
                        pos["agent_id"],
                        pos["symbol"],
                        "funding" if funding >= 0 else "funding_credit",
                        pos["entry_price"],
                        abs(qty),
                        f"[auto-funding] {'paid' if funding > 0 else 'received'} ${abs(funding):.4f} "
                        f"({rate * 100:.4f}% of notional)",
                        int(time.time()),
                        utc_now_iso_z(),
                        utc_now_iso_z(),
                    ),
            ...
            conn.commit()
            if charged:
                print(f"[Perp Funding] charged={charged} rate={rate}")
        except Exception as exc:
            print(f"[Perp Funding Error] {exc}")
        finally:
            try:
                conn.close()
            except Exception:
...
BACKGROUND_TASK_REGISTRY = {
    ...
    "position_risk_management": position_risk_management_loop,
    "perp_funding": perp_funding_loop,
