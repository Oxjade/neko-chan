...
def _update_position_from_signal(
    ...
    # Store a mark at write time so leaderboard/profit surfaces agree with the
    # positions endpoint immediately (worker refreshes it within 5 minutes).
    mark_price = current_price if current_price is not None else price
    ...
    if action_lower == "buy":
        ...
        if current_qty > 0:
            ...
            cursor.execute("""
                UPDATE positions SET quantity = ?, entry_price = ?, opened_at = ?, current_price = ?
                WHERE id = ?
            """, (new_qty, new_entry_price, executed_at, mark_price, position_id))
            print(f"[Position] {symbol}: increased long position to {new_qty}")
        else:
            # Create new long position
            if leader_id:
                cursor.execute("""
                    INSERT INTO positions (agent_id, symbol, market, token_id, outcome, side, quantity, entry_price, opened_at, leader_id, current_price, stop_loss, take_profit, leverage)
                    VALUES (?, ?, ?, ?, ?, 'long', ?, ?, ?, ?, ?, ?, ?, ?)
                """, (agent_id, symbol, market, token_id, outcome, quantity, price, executed_at, leader_id, mark_price, stop_loss, take_profit, leverage))
                print(f"[Position] {symbol}: created copied long position {quantity} from leader {leader_id}")
            else:
                cursor.execute("""
                    INSERT INTO positions (agent_id, symbol, market, token_id, outcome, side, quantity, entry_price, opened_at, current_price, stop_loss, take_profit, leverage)
                    VALUES (?, ?, ?, ?, ?, 'long', ?, ?, ?, ?, ?, ?, ?)
                """, (agent_id, symbol, market, token_id, outcome, quantity, price, executed_at, mark_price, stop_loss, take_profit, leverage))
