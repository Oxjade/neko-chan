...
def test_simple_flow_registers_bot(monkeypatch, registry):
    """token-less: name -> agent + token-less bot registered (platform mocked)."""
    from handlers import common
    ...
    # S_NAME == 1 -> the single name handler
    ...
    # token-less: no @BotFather token stored for the new bot
    assert registry.bot_token(bots[0]["id"]) is None
